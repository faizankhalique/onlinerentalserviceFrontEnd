...
import {
  approvedHouseRentRequest,
  sendEmail
} from "../../services/properties/house/houseBookingService";
import Input from "./../../common/input";
import TextArea from "./../../common/textArea";
import Select from "./../../common/select";
class HouseRentRequestDetails extends Component {
  state = {
    account: {
      receipent: "",
      subject: "",
  ...
  componentDidMount() {
    const { houseRentRequest } = this.props.location.state;
    const renter = houseRentRequest.renter;
    const house = houseRentRequest.house;
    const houseOwner = houseRentRequest.owner;
    const houseImage = houseRentRequest.house.houseImages[0];
    const houseImages = houseRentRequest.house.houseImages;
    this.setState({
      houseRentRequest,
      renter,
      houseImage,
  ...
  handleApprovedRequest = async requestId => {
    ...
    if (isConfirm) {
      try {
        const { data: result } = await approvedHouseRentRequest(requestId);
        if (result) {
          ...
          >
            <div className="col-md-6">
              ...
              <button
                ...
                onClick={() => {
                  this.handleApprovedRequest(houseRentRequest._id);
                }}
              >
                Approved Request
              </button>
...
export default HouseRentRequestDetails;
